...
def sketch_retrieval(model_path, im, image_feats):
    ...
    num, chns, rows, cols = transformed.shape
    # reshape data layer
    model_path.blobs['data'].reshape(*(num, chns, rows, cols))
    # forward
    output = model_path.forward(data=transformed.astype(np.float32, copy=False))
