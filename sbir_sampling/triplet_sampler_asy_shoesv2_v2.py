...
class TripletSamplingLayer(object):
    def setup(self, sketch_dir, image_dir, triplet_path, mean, hard_ratio, batch_size, phase, mode, att_flag=False):
        ...
        self.create_sample_fetcher(sketch_dir, image_dir, triplet_path, mean, hard_ratio, batch_size, phase, mode, att_flag)

    def create_sample_fetcher(self, sketch_dir, image_dir, triplet_path, mean, hard_ratio, batch_size, phase, mode, att_flag):
...
class TripletSamplingDataFetcher(Process):
    def __init__(self, queue, sketch_dir, image_dir, triplet_path, mean, hard_ratio, batch_size, phase, proc_id, mode, att_flag=False):
    ...
    def get_next_batch(self):
        ...
        if self.att_flag:
            att_anc_list = []; att_pos_list = []; att_neg_list = []
        ...
        if self._phase == 'TRAIN' and (self.mode=='whole-part' or self.mode=='part'):
            pos_inds, neg_inds, bbox_inds = sample_triplets_with_filter(anc_inds, self.triplets, self.bbox,
                                                                    self.neg_list, self.hard_ratio, self._phase)
        else:
            try:
                pos_inds, neg_inds = sample_triplets(anc_inds, self.triplets, self.neg_list, self.hard_ratio, self._phase)
        ...
        # print('anc')
        # print(anc_batch[0].shape)
        # print('pos')
        # print(pos_batch[0].shape)
        # print('neg')
        # print(neg_batch[0].shape)
        if self._phase == 'TRAIN' and (self.mode=='whole-part' or self.mode=='part'):
            ...
            if self.att_flag:
                ...
                att_pos_batch = np.reshape(att_pos_list, (len(anc_inds), dim_att))
                att_neg_batch = np.reshape(att_neg_list, (len(anc_inds), dim_att))
                self._queue.put((anc_batch_trans, pos_batch_trans, neg_batch_trans, att_anc_batch, att_pos_batch, att_neg_batch))
            else:
                self._queue.put((anc_batch_trans, pos_batch_trans, neg_batch_trans))
...
if __name__ == '__main__':
    ...
    mode = 'whole'
    att_flag = True
    sketch_dir = '/import/vision-ephemeral/QY/code/triplet_with_bbox/data/%s/%s_sketch_db_%s.mat' % (subset, subset, phase.lower())
    image_dir = '/import/vision-ephemeral/QY/code/triplet_with_bbox/data/%s/%s_edge_db_%s.mat' % (subset, subset, phase.lower())
    triplet_path = '/import/vision-ephemeral/QY/code/triplet_with_bbox/data/%s/%s_annotation.json' % (subset, subset)
    sketch_dir_te = '/import/vision-ephemeral/QY/code/triplet_with_bbox/data/%s/%s_sketch_db_%s.mat' % (subset, subset, phase_te.lower())
    image_dir_te = '/import/vision-ephemeral/QY/code/triplet_with_bbox/data/%s/%s_edge_db_%s.mat' % (subset, subset, phase_te.lower())
    triplet_sampler = TripletSamplingLayer()
    triplet_sampler.setup(sketch_dir_te, image_dir_te, triplet_path, mean, hard_ratio, batch_size, phase_te, mode, att_flag)
